...
* **HTTPS with optional HTTP-to-HTTPS Redirection**<br>the main server handles HTTPS only as it becomes more and more difficult to serve browsers with HTTP-only content (even locally). If desired, an additional auxiliary HTTP server may be started which redirects incoming requests to its HTTPS counterpart
